...
object W2V {
  def main(args: Array[String]): Unit = {
    ...
    val documents = sc.wholeTextFiles(path = "data/abstracts", minPartitions = 2)

    val input1 = documents.map(abs => {
      abs._2
    })

    val input = input1.map(line => line.split(" ").toSeq)

    val modelFolder = new File("myModelPath")

    if (modelFolder.exists()) {
      val sameModel = Word2VecModel.load(sc, "myModelPath")
      val synonyms = sameModel.findSynonyms("behavior", 5)
      ...
      val synonyms = model.findSynonyms("behavior", 5)
